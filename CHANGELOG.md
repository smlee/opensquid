...
- The bot-token `.env` is canonically `~/.opensquid/.env`. `locateEnvFile` now **auto-migrates** a
  ...
  anymore (no token loss; the one remaining reference exists only to _move_ the file away). Also removed
...
- `prepublishOnly` now cleans `dist/` before building (`pnpm clean && pnpm schemas && pnpm build`), plus
  a `clean` script. `tsc` is additive — it never deletes outputs whose source was removed, so `dist/` had
  accumulated the retired `anti-drift/*`, `engine-client.js`, `rag/backends/loop_engine.js`, and 56
  compiled `*.test.js` files (no live `src/`), all shipping in the npm tarball. The clean runs at PUBLISH
  (not in the shared `build`) so it never wipes `dist/` out from under the e2e tests that invoke `pnpm build`
  concurrently. `build` also `chmod +x`'s the bin targets (a fresh compile doesn't set the execute bit the
  linked-dev binary needs; npm sets it on install for published users). Verified `npm pack --dry-run` after a
  clean build lists none of the dead files. Found in the CLI/stale audit; recorded in `docs/ARCHITECTURE.md` §8.
...
`opensquid handoff` generates the four canonical handoff surfaces DETERMINISTICALLY
from disk state — "hand-off needs all the information": (a) an idempotent
`<umbrella>/docs/handover-<date>-session-<sid8>-auto.md` with FSM history, active task

- both phase ledgers, audit verdict heads (incl. UNRESOLVED bullets — the precise
  resume point), spawn-ledger + attestation tails, per-repo git status/unpushed, open
  work-graph issues, artifact paths WITH content hashes (disk truth, never narrative),
  and MECHANICAL resume steps (research-flow-first when mid-flow; commit-the-doc(s)
  last); (b) a marker-managed MEMORY.md resume block (bytes outside the markers never
  touched); (c) a `handoff-<sid8>` work-graph upsert; (d) a best-effort chat digest.
  Three triggers: the command (primary), a SessionEnd backup writer (placed BEFORE the
  state-destroying archive/clear tail), and — the wedge-proof piece — SessionStart LAZY
  GENERATION: the FU.3 project pointer still names the dead session at fresh-session
  start, so even a kill-9'd session gets its handoff generated by the successor from
  disk, then injected via additionalContext (once per session). The manual-handover era
  ends. The narrative LLM layer is a deliberate follow-up — never load-bearing.
