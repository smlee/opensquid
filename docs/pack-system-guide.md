...
| -------------- | ---------------------------------------------- | -------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `name`         | `z.string().min(1)` (`skill.ts:293`)           | — (required)                                                   | Skill id; also the classification label the router uses (`skill_router.ts:71`, presented to the classifier as `name: prose` at `skill_router.ts:80`).                                                                                       |
...
### `prose` — its role in selection (skill_prefilter.ts / skill_router.ts)

`prose` is the description embedded/classified during dynamic-load selection — a two-tier funnel that biases toward **recall** ("load everything" on any failure):

1. **Prefilter** (`skill_prefilter.ts:75-118`): embeds the task subject and each skill's `prose` (falling back to `name` when prose is absent — `skill_prefilter.ts:101`), ranks by cosine similarity, returns top-K (default 10, `skill_prefilter.ts:82`). If the embedder is unreachable on the subject → returns **all** skills (`skill_prefilter.ts:89-93`). This is why production packs should author prose: `name` is a weak signal (`skill_prefilter.ts:98-100`).
2. **Router** (`skill_router.ts:59-114`): asks the `fast_classifier`-aliased LLM (never a vendor model name — `skill_router.ts:11-16`) which shortlisted skills are needed, presenting each as `name: prose` (`skill_router.ts:80`). Classifier error / not-ok / `UNCERTAIN` / `NONE` → return **all** candidates (`skill_router.ts:98,106`); a parsed comma-separated list → the matching subset (`skill_router.ts:112-113`).

This funnel is an _optimization, not a gate_: the worst case (both layers down) is "load every skill," the same state you'd be in without the layer. The hard filter is the dispatcher's trigger-kind + requires gate, NOT prose similarity.
...
| `cached_audit`      | `{ cache_key, model, prompt, timeout_ms? }` (`cached_audit.ts:46-53`)                        | `ok(verdict_string)`; caches ONLY a real verdict (output contains `"VERDICT:"`), keyed by `sha256(prompt)` in session state; timeout/error returned as `err` and NEVER cached (`cached_audit.ts:124-179`) | The SCOPE/AUTHOR gate audit. Re-firing on UNCHANGED content (same prompt hash) returns the cached verdict with NO spawn — the F0c spawn-exhaustion fix. Emits a spawn ledger (`cached_audit.ts:84-110`). |
