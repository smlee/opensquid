...
function run(steps: ProcessStep[], event: Event, packFsm: Fsm): Promise<RuleResult> {
...
describe('fullstack-flow pause-guard (T2.9)', () => {
  ...
  it('AskUserQuestion IN SCOPE (phase == scope) → ALLOWED (no verdict)', async () => {
    const r = await run(await ruleSteps(), toolCall('AskUserQuestion'), fsmAt('scope'));
    expect(r.kind).toBe('no_verdict');
  });

  it('Stop IN SCOPE (phase == scope) → ALLOWED (no verdict)', async () => {
    const r = await run(await ruleSteps(), toolCall('Stop'), fsmAt('scope'));
    expect(r.kind).toBe('no_verdict');
  });

  it('AskUserQuestion POST-SCOPE (phase != scope) → BLOCKED', async () => {
    const r = await run(await ruleSteps(), toolCall('AskUserQuestion'), fsmAt('code'));
  ...
  it('Stop POST-SCOPE (phase != scope) → BLOCKED', async () => {
    const r = await run(await ruleSteps(), toolCall('Stop'), fsmAt('author'));
    expect(r.kind).toBe('verdict');
    if (r.kind === 'verdict') expect(r.verdict.level).toBe('block');
  });

  it('a non-pause tool POST-SCOPE → NEVER blocked (no verdict)', async () => {
    const r = await run(await ruleSteps(), toolCall('Edit'), fsmAt('code'));
    expect(r.kind).toBe('no_verdict');
  });

  it('a non-pause tool IN SCOPE → NEVER blocked (no verdict)', async () => {
    const r = await run(await ruleSteps(), toolCall('Bash'), fsmAt('scope'));
    expect(r.kind).toBe('no_verdict');
  });
});
