...
export interface ScopeExtract {
  ...
  /** AD.4 ask-grounded roots: a design-id + the verbatim ask span that should verify it. */
  scopeElements: { designId: string; askSpan: string }[];
  ...
  /** T2.5 autoDecompose edges: `element` depends on `dependsOn` (a `[needs: M]` ref). */
  deps: { element: string; dependsOn: string }[];
}
...
/** a declared dependency: `[needs: M]`. */
const NEEDS = /\[needs:\s*(\d+)\]/g;
...
export async function extractScope(p: string): Promise<ScopeExtract | null> {
  ...
  for (const m of text.matchAll(NUMBERED)) {
    ...
    scopeElements.push({ designId: id, askSpan });
    tasks.push({
      designId: id,
    ...
    for (const d of body.matchAll(NEEDS))
      deps.push({ element: id, dependsOn: `scope-${d[1] ?? ''}` });
  }
