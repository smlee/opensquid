...
describe('coverage report-only over the live tree (CFD.1)', () => {
  it('reports each seeded requirement status deterministically (report-only, no LLM/subprocess)', () => {
    ...
    for (const id of [
      ...
      'R-FSCOPE-ENGAGEMENT',
      'R-FSCOPE-LANE',
      'R-FSCOPE-STRICT-ACTIVE',
      'R-FSCOPE-MIRROR-CONTINUITY',
      'R-FSCOPE-PI-CONTINUATION',
    ]) {
      expect(byId[id]).toBe('met');
    }
    expect(a.results.length).toBe(112); // 105 prior requirements + 7 fullstack /scope requirements
  }, 30_000);
});
